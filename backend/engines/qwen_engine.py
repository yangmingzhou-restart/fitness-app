...
6. 根据你的经验估算每种食物的总热量（kcal）和每千克热量（kcal/kg）

重要：图片中的食物均为已烹饪/煮熟后的状态。你必须按照熟食的营养成分来估算。
例如：米饭是已煮熟的状态（约28-30g碳水/100g），不是生米；肉类是已煎/煮/炒熟的状态。

注意：你估算的热量值仅用于日志比对参考，系统最终会使用蛋白质×4+碳水×4+脂肪×9的公式重新计算热量。
    ...
    async def analyze(
        ...
        try:
            ...
            for item in foods_data:
                ...
                # 本地计算热量 (4-4-9 公式)，不依赖 AI 返回的热量值
                local_estimated_cal = protein_g * KCAL_PER_G_PROTEIN + carbs_g * KCAL_PER_G_CARBS + fat_g * KCAL_PER_G_FAT
                local_cal_per_kg = (protein_per100 * KCAL_PER_G_PROTEIN + carbs_per100 * KCAL_PER_G_CARBS + fat_per100 * KCAL_PER_G_FAT) * 10

                ai_calories = item.get("estimatedCalories")
                ai_cal_per_kg = item.get("caloriesPerKg")

                food_name = item.get("name", "")
                logger.info(
                    f"[热量对比] {food_name} | "
                    f"AI返回: 热量={ai_calories}, 每kg={ai_cal_per_kg} | "
                    f"本地计算: 热量={round(local_estimated_cal, 1)}, 每kg={round(local_cal_per_kg, 1)} | "
                    f"公式: P={protein_g}×4 + C={carbs_g}×4 + F={fat_g}×9 = {round(local_estimated_cal, 1)}kcal"
                )
                ...
                food = FoodItem(
                    ...
                    caloriesPerKg=round(local_cal_per_kg, 1),
                    estimatedWeightG=float(item.get("estimatedWeightG", 0)),
                    estimatedCalories=round(local_estimated_cal, 1),
